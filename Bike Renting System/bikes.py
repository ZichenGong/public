...
def clean_data(data: List[list]) -> None:
    ...
    for char in data:
        for i in range(len(char)):
            if not char[i].isalnum():
                char[i] = float(char[i])
                if round(char[i]) == char[i]:
                    char[i] = round(char[i])
                else:
                    char[i] = char[i]
            elif char[i].isdigit():
...
def has_kiosk(station: "Station") -> bool:
    ...
    return NO_KIOSK not in station[NAME]
...
def get_station_info(station_id: int, stations: List["Station"]) -> list:
    ...
    for element in stations:
        if station_id == element[ID]:
            returned_info.append(element[NAME])
            returned_info.append(element[BIKES_AVAILABLE])
            returned_info.append(element[DOCKS_AVAILABLE])
            returned_info.append(has_kiosk(element))
...
def get_nearest_station(lat: float, lon: float, with_kiosk: bool,
    ...
    if with_kiosk:
        for i in stations:
            if has_kiosk(i):
                num_dist = get_lat_lon_distance(lat, lon, \
                                                i[LATITUDE], \
                                                i[LONGITUDE])
                distance.append(i[ID])
                distance.append(num_dist)
                station_list.append(distance)
                distance = []
        dist = []
...
def rent_bike(station_id: int, stations: List["Station"]) -> bool:
    ...
    for element in stations:
        if station_id in element and element[BIKES_AVAILABLE] > 0:
            element[BIKES_AVAILABLE] = element[BIKES_AVAILABLE] - 1
            element[DOCKS_AVAILABLE] = element[DOCKS_AVAILABLE] + 1
            return True
    return False
...
def return_bike(station_id: int, stations: List["Station"]) -> bool:
    ...
    for element in stations:
        if station_id in element and element[DOCKS_AVAILABLE] > 0:
            element[BIKES_AVAILABLE] = element[BIKES_AVAILABLE] + 1
            element[DOCKS_AVAILABLE] = element[DOCKS_AVAILABLE] - 1
            return True
    return False
...
def redistribute_bikes(stations: List["Station"]) -> int:
    ...
    cap_total = get_total(CAPACITY, stations)
    bikes_total = get_total(BIKES_AVAILABLE, stations)
    bikes_percentage = bikes_total / cap_total
    result = 0
    for m in stations:
        bike_num = round(m[CAPACITY] * bikes_percentage)
        dif_num = m[BIKES_AVAILABLE] - bike_num
        m[DOCKS_AVAILABLE] -= round(m[CAPACITY] * bikes_percentage) \
            - m[BIKES_AVAILABLE]            
        m[BIKES_AVAILABLE] = round(m[CAPACITY] * bikes_percentage)
        result += dif_num
    return result
